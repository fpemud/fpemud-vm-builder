...
class PluginObject(FvmPluginOs):
	...
	def getUsage(self):
		ret = ""
		ret += "Setup options:\n"
		ret += "  os=Microsoft.Windows.XP.Professional.SP3.X86.zh_CN\n" 
		ret += "  os=Microsoft.Windows.XP.Professional.SP1.X86_64.zh_CN\n"
		ret += "Configure options:\n"
		ret += "  screen-resolution=RESOLUTION\n"
		ret += "  explorer-view=default|thumbnail|title|icon|list|detail\n"
		ret += "  explorer-option=file-ext:show|hide;hidden-file:show|hide;extra-tip:show|hide\n"
		ret += "  explorer-search-option=balloon-prompt:on|off;auto-completion:on|off;animation:on|off;indexing:on|off\n"
		ret += "  desktop-theme=NAME\n"
		ret += "  desktop-performance=effect|speed\n"
		ret += "  desktop-option=sort-icon:on|off\n"
		ret += "  start-menu-option=style:winxp|classic;auto-hide:on|off;inactive-tray-icon:hide|show;group-task:on|off;quick-bar:on|off;balloon-tips:on|off\n"
		ret += "  ie-main-page=default|blank|URL\n"
		ret += "  wmp-option=minimize-to-taskbar:on|off\n"
		ret += "  control-panel-style=category|classic\n"
		ret += "  error-report=enable|disable\n"
		ret += "  taskmgr-option=auto-start:on|off;hide-when-minimized:on|off\n"
		ret += "  sys-patch=wmp11;flash-player;jre;dot-net-framework;directx\n"
		ret += "  desktop-shortcut=position:desktop|quick-launch-bar;name:NAME;target:TARGET;accelerator:ACCEL\n"
		ret += "Execute options:\n"
		ret += "  clear-cruft\n"
		ret += "  check-virus\n"
		ret += "  performance-test\n"
		return ret
	...
	def doConfigure(self, param, vmInfo, optList):
		...
		# get FvmWork list by optList
		workList = []
		for opt in optList:
			t = opt.split("=")

			if t[0] == "screen-resolution":
				work = self._doJobSetScreenResolution(opt, t[1])
				workList.append(work)
			elif t[0] == "explorer-view":
				work = self._doJobSetExplorerView(opt, t[1])
				workList.append(work)
			elif t[0] == "explorer-option":
				work = self._doJobSetExplorerOption(t[1])
				workList.append(work)
			elif t[0] == "explorer-search-option":
				work = self._doJobSetExplorerSearchOption(t[1])
				workList.append(work)
			elif t[0] == "desktop-theme":
				work = self._doJobSetDesktopTheme(t[1])
				workList.append(work)
			elif t[0] == "desktop-performance":
				work = self._doJobSetDesktopPerformance(t[1])
				workList.append(work)
			elif t[0] == "desktop-option":
				work = self._doJobSetDesktopOption(t[1])
				workList.append(work)
			elif t[0] == "start-menu-option":
				work = self._doJobSetStartMenuOption(t[1])
				workList.append(work)
			elif t[0] == "ie-main-page":
				work = self._doJobSetIeMainPage(t[1])
				workList.append(work)
			elif t[0] == "wmp-option":
				work = self._doJobSetWmpOption(t[1])
				workList.append(work)
			elif t[0] == "control-panel-style":
				work = self._doJobSetControlPanelStyle(t[1])
				workList.append(work)
			elif t[0] == "error-report":
				work = self._doJobSetErrorReport(t[1])
				workList.append(work)
			elif t[0] == "taskmgr-option":
				work = self._doJobSetTaskmgrOption(t[1])
				workList.append(work)
			elif t[0] == "sys-patch":
				retList = self._doJobInstallPatch(osName, t[1])
				workList += retList
			elif t[0] == "desktop-shortcut":
	...
	def mergeCfgOptList(self, param, vmCfgWin, optList):
		ret = vmCfgWin.os.cfgOptList
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "screen-resolution=")
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "explorer-view=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "explorer-option=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "explorer-search-option=")
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "desktop-theme=")
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "desktop-performance=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "desktop-option=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "start-menu-option=")
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "ie-main-page=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "wmp-option=")
		ret = CfgOptUtil.mergeCfgOpt(ret, optList, "error-report=")
		ret = CfgOptUtil.mergeCfgOptWithSubKeyValue(ret, optList, "taskmgr-option=")
		ret = CfgOptUtil.mergeCfgOptWithSubValue(ret, optList, "sys-patch=")
		ret = CfgOptUtil.mergeCfgOptWithKeyAdd(ret, optList, "desktop-shortcut=")
		return ret
	...
	def doExecute(self, param, vmInfo, optList):
		...
		for opt in optList:
			if opt == "clear-cruft":
				assert False
			elif opt == "check-virus":
				work = self._doJobCheckVirus()
				workList.append(work)
